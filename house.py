...
class House:
    def __init__(self, url, address, price, locs):
        self.url = url
    ...
    def get_travel_dist(self, start, ella_dist=True):
        ...
        try:
            ...
            public_transport_div = WebDriverWait(driver, 10, poll_frequency=0.1).until(
    ...
    def find_area_info(self):
        ...
        for coords in [[-1, -1], [-1, 1], [1, 1], [1, -1]]:
            i, j = coords
            poly += f"{str(lat + i * crime_radius_deg)},{str(long + j * crime_radius_deg)}"
            poly += ":"

        try:
            url = f'https://data.police.uk/api/crimes-street/all-crime?poly={poly[:-1]}'
    
            x = requests.post(url)
            data = x.json()
    
            crime_number = len(data)
            crime_ratio = crime_number / AVG_LONDON
            print(f"crime number in last year in 1 mile radius: {crime_number}, crime ratio compared to UK average: {crime_ratio}")
    
            self.crime_rate = len(data)
        except Exception:
            pass

        try:
            url = f'https://data.police.uk/api/crimes-street/violent-crime?poly={poly[:-1]}'
            x = requests.post(url)
            data = x.json()
            self.violent_crime_rate = len(data)
    
            print(f"violent crime number in last year in 1 mile radius: {self.violent_crime_rate}")
        except Exception:
            pass
        ...
        try:
            ...
            try:
                gyms_table = driver.find_element_by_xpath("//td[contains(text(),'Health and Fitness Gym')]")
                print("Closest gym: ",gyms_table.text)
                self.closest_gym = gyms_table.text
            except:
                consent_btn = driver.find_element_by_xpath("//p[contains(text(),'Consent')]")
                consent_btn.click()
                try:
                    gyms_table = driver.find_element_by_xpath("//td[contains(text(),'Heath and Fitness Gym')]")
                    print("Closest gym: ", gyms_table.text)
                    self.closest_gym = gyms_table.text
                except:
                    pass
