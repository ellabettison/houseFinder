...
class HouseFinder:
    def __init__(self, max_price, furnished, locs, locs_travel_time, min_bedrooms=1, max_shop_distance=2, max_any_shop_distance=0.5,
                 max_train_distance=3, max_crime_rate=3, max_violent_crime_rate=2, search_radius=3):
        ...
        for _ in range(30):
            try:
                ...
                for house in houses_list:
                    url = house.find_element(By.CSS_SELECTOR, 'a[data-test="property-details"]')
                    address = house.find_element(By.CLASS_NAME, 'propertyCard-address')
                    price = house.find_element(By.CLASS_NAME, 'propertyCard-priceValue')

                    new_house = House(url=url.get_attribute("href"), address=address.text,
                                      price=price.text, locs=self.locs)
    ...
    def find_suitable_houses(self):
        ...
        for i, house in enumerate(houses):
            ...
            house_dist_list = [f"\t{self.locs[i].upper()} DIST: {dists[i] if i < len(dists) else 'Skipped'} mins\t|" for i in range(len(self.locs))]
    ...
    def find_houses_info(self, suitable_houses):
        ...
        dataframe = pd.DataFrame(
            columns=['Address', 'Price', 'Crime Rate', 'Closest Shop', 'Shop Distance',
                     'Closest Station', 'Station Distance', 'Closest Gym', 'URL'] + [f"{loc} Time" for loc in self.locs])
        ...
        for house in final_suitable_houses:
            ...
            dataframe = dataframe.append({"Address": house.address,
                                          ...
                                          "Closest Gym": house.closest_gym.replace('\n', ''),
                                          "URL": house.url} | dists_dict, ignore_index=True)
